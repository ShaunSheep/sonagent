...
放入视频后，服务器启动时自动执行以下逻辑：

```
用户放入视频 → public/videos/
    ↓
syncVideosWithDirectory()
    ├── 自动创建 public/videos/（如不存在）
    ├── 自动创建/更新 videos.json（如不存在）
    ├── 扫描目录中的 .mp4 .avi .mov .mkv 文件
    └── 自动合并写入 videos.json（新增文件自动添加，已有文件保留配置）
```

> `videos.json` 无需手动编辑，也无需提交到仓库（已加入 `.gitignore`）。

---

### 🖼️ 生成视频缩略图（可选但推荐）

视频卡片默认显示 `🎬` emoji，运行缩略图脚本后会显示真实视频封面。
...
python generate_thumbnails.py
    ↓
读取 videos.json → 检测无缩略图的视频
    ↓
用 OpenCV 提取视频第 0.5 秒帧
    ↓
...
> 每次新增视频后重新运行即可生成新缩略图。生成的缩略图文件已加入 `.gitignore`，不会污染仓库。
...
### 学习计划功能
当孩子说"看视频"时，先判断今天是否已完成学习计划，未完成则引导先学习。
